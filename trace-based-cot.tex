...
\begin{tabular}{@{}p{0.29\textwidth}@{\hspace{0.01\textwidth}|}@{\hspace{0.01\textwidth}}p{0.33\textwidth}@{\hspace{0.01\textwidth}|}@{\hspace{0.01\textwidth}}p{0.33\textwidth}@{}}
\toprule
\textit{Code} & \textit{LLM-Generated CoT (Hallucinated)} & \textit{Ours (Trace-Grounded)} \\
\midrule
\vspace{-0.3cm}
...
\end{lstlisting}
&
\colorbox{errorred!8}{\parbox{0.98\linewidth}{\scriptsize
\vspace{0.08cm}
1. First, we initialize our search boundaries. We set left=0 to start at the beginning of the array, and right=4 which is the last index.\\[0.05cm]
2. Now we need to find the middle point. We calculate mid=2, which gives us the center of our current search range.\\[0.05cm]
3. Here's where we check if we're going uphill or downhill. We compare arr[2] with \textcolor{errorred}{\textbf{arr[1]}} to see which direction the peak might be.\\[0.05cm]
4. Since arr[2] is \textcolor{errorred}{\textbf{greater than}} arr[1], this means we're still climbing up the array, so the peak must be further to the \textcolor{errorred}{\textbf{right}}. We update our search boundary accordingly.\\[0.05cm]
5. We keep repeating this process, narrowing down our search until left equals right, which means we've found our target.\\[0.05cm]
6. Finally, we return the peak index.\\[0.08cm]
\textcolor{errorred}{\xmark\ Wrong: compares arr[2] with arr[1] instead of arr[3], moves in wrong direction}
\vspace{0.08cm}
}}
&
\parbox{0.98\linewidth}{
\colorbox{gray!15}{\parbox{0.98\linewidth}{\ttfamily\scriptsize
\vspace{0.05cm}
\colorbox{initcolor}{arr=[1,3,5,4,2], left=0, right=4}\\
\colorbox{processcolor}{mid=2, arr[2]=5, arr[3]=4}\\
\colorbox{processcolor}{5>4 $\Rightarrow$ right=2}\\
\colorbox{processcolor}{mid=1, arr[1]=3, arr[2]=5}\\
\colorbox{processcolor}{3<5 $\Rightarrow$ left=2}\\
\colorbox{finalcolor}{left==right, return 2}
\vspace{0.05cm}
}}\\[0.1cm]
\colorbox{correctgreen!8}{\parbox{0.98\linewidth}{\scriptsize
\vspace{0.08cm}
\colorbox{initcolor}{\parbox{0.96\linewidth}{1. \textbf{Init:} left=0, right=4, arr=[1,3,5,4,2]}}\\[0.05cm]
\colorbox{processcolor}{\parbox{0.96\linewidth}{2. \textbf{Iter 1:} mid=2, compare arr[2]=5 with arr[3]=4}}\\
\colorbox{processcolor}{\parbox{0.96\linewidth}{3. Since 5\textcolor{correctgreen}{\textbf{>}}4, peak in left half $\Rightarrow$ right=2}}\\
\colorbox{processcolor}{\parbox{0.96\linewidth}{4. \textbf{Iter 2:} mid=1, compare arr[1]=3 with arr[2]=5}}\\
\colorbox{processcolor}{\parbox{0.96\linewidth}{5. Since 3\textcolor{correctgreen}{\textbf{<}}5, peak in right $\Rightarrow$ left=2}}\\[0.05cm]
\colorbox{finalcolor}{\parbox{0.96\linewidth}{6. \textbf{Done:} left==right==2, return 2}}\\[0.05cm]
\textcolor{correctgreen}{\cmark\ Each step verified by trace}
\vspace{0.08cm}
}}
}
\\
...
\end{tabular}
\vspace{-0.1cm}
\caption{Visual mapping between execution trace and CoT reasoning. In our trace-grounded approach (right), colored boxes show how trace sections map to CoT: \colorbox{initcolor}{yellow=initialization}, \colorbox{processcolor}{blue=loop iterations}, \colorbox{finalcolor}{green=final state}. Variable values in CoT exactly match trace. LLM-generated CoT (middle) is verbose but hallucinates wrong array indices and search direction.}
