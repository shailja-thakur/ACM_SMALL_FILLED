...
\vspace{0.05cm}
The function starts by setting up search boundaries with left=0 and right=4 to span the entire array. Then it enters a loop to narrow down where the peak element is located. In each iteration, it calculates the midpoint and checks whether we're on an ascending or descending slope. When we compare arr[2] with \textcolor{errorred}{\textbf{arr[1]}}, since arr[2] is larger, this indicates we're climbing upward, so the peak must be to the \textcolor{errorred}{\textbf{right}}. The algorithm continues adjusting the boundaries until they converge at the peak location.\\[0.05cm]
\textcolor{errorred}{\xmark\ Hallucinates wrong indices \& direction}
\vspace{0.05cm}
}}
&
\parbox{0.98\linewidth}{
\colorbox{gray!15}{\parbox{0.98\linewidth}{\ttfamily\tiny
\vspace{0.03cm}
\colorbox{initcolor}{Line 2~~left = 0}\\
\colorbox{initcolor}{Line 3~~right = 4}\\
\colorbox{processcolor}{Line 5~~mid = 2}\\
\colorbox{processcolor}{Line 6~~arr[2] = 5, arr[3] = 4}\\
\colorbox{processcolor}{Line 9~~right: 4 -> 2}\\
\colorbox{processcolor}{Line 5~~mid = 1}\\
\colorbox{processcolor}{Line 6~~arr[1] = 3, arr[2] = 5}\\
\colorbox{processcolor}{Line 7~~left: 0 -> 2}\\
\colorbox{finalcolor}{Line 10~return 2}
\vspace{0.03cm}
}}\\[0.08cm]
\colorbox{correctgreen!8}{\parbox{0.98\linewidth}{\scriptsize
\vspace{0.05cm}
\colorbox{initcolor}{\parbox{0.96\linewidth}{\scriptsize We begin with left=0 and right=4, covering the full array [1,3,5,4,2].}}\\[0.03cm]
\colorbox{processcolor}{\parbox{0.96\linewidth}{\scriptsize First iteration: midpoint is index 2 (value 5). Comparing with arr[3]=4 shows we're past the peak, so we search the left portion by setting right=2.}}\\[0.03cm]
\colorbox{processcolor}{\parbox{0.96\linewidth}{\scriptsize Second iteration: midpoint is now index 1 (value 3). Comparing with arr[2]=5 shows the peak is ahead, so we move left=2.}}\\[0.03cm]
\colorbox{finalcolor}{\parbox{0.96\linewidth}{\scriptsize The boundaries converge at index 2, which contains our peak value of 5.}}\\[0.03cm]
\textcolor{correctgreen}{\cmark\ Each step matches trace}
\vspace{0.05cm}
}}
}
\\
...
\end{tabular}
\vspace{-0.1cm}
\caption{Comparison of LLM-hallucinated vs trace-grounded CoT. Our approach (right) grounds each reasoning step in actual execution: the pysnooper-style trace shows line numbers and variable states, which are then narrated naturally. The colored boxes map trace sections (yellow=init, blue=iterations, green=final) to CoT. LLM-generated CoT (middle) produces plausible-sounding but factually wrong reasoning about array indices and search direction.}
