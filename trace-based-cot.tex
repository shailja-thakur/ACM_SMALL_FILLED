...
\begin{enumerate}
    ...
    \item \textbf{Concept-First Curriculum Synthesis}: A novel curriculum-driven generation approach that provides fine-grained control over problem complexity and algorithmic diversity, enabling systematic coverage from basic constructs to advanced algorithms.
...
\textbf{Results and Analysis.} Table~\ref{tab:verification_ablation} presents the results. Our full approach achieves the highest performance across all benchmarks, with \texttt{Qwen2.5-Coder-7B} reaching \textbf{68.2\%} on LiveCodeBench, \textbf{59.7\%} on CruxEval Output, and \textbf{61.9\%} on CruxEval Input. To understand what drives this performance, we analyze the cost of removing each component:

\textit{The Cost of Weak Verification.} Our minimal execution variant achieves 64.1\% on LiveCodeBench (a -4.1 point drop), demonstrating that verification quality degrades without sufficient candidate diversity. This confirms the value of Dual Agreement's statistical consensus mechanism for filtering out edge-case failures and ensuring robust correctness.

\textit{The Cost of Abandoning Execution Grounding.} The LLM-Generated CoT approach performs substantially worse at 59.3\% on LiveCodeBench (a -8.9 point drop from our full approach). This gap validates our core thesis: LLM-generated explanations, while plausible, lack the factual accuracy guaranteed by direct trace translation. The model learns to imitate superficially correct reasoning patterns rather than verifiable execution semantics.

\textit{The Cost of Removing Rationales Entirely.} Removing CoT rationales altogether (Q\&A Only) causes a severe performance drop to 52.7\% on LiveCodeBench (a -15.5 point drop). This validates that trace-grounded reasoning chains provide crucial learning signal beyond mere input-output mappings—the model benefits from understanding \textit{how} outputs arise from inputs, not just the final correspondence.

\textit{The Baseline Context.} The few-shot baseline (48.1\%, 47.0\%, 48.8\%) performs only marginally better than the base model (46.3\%, 45.3\%, 47.5\%), highlighting the limited value of prompting alone and reinforcing the necessity of fine-tuning on verified data.

\textbf{Conclusion.} The results unequivocally demonstrate that \textit{both} components—execution grounding and robust verification—are essential. The method used to generate CoT rationales is not merely a technical detail; it fundamentally determines whether models learn verifiable reasoning or superficial imitation. Execution-grounded rationales, validated through statistical consensus, provide the most reliable foundation for training robust code reasoning capabilities.
