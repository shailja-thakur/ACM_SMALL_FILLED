...
\begin{abstract}
  Chain-of-Thought (CoT) fine-tuning for code suffers from a critical flaw: the reasoning steps in synthetic
     ...
     by construction. Each reasoning step can be verified against the trace to confirm correct variable values,
     state transitions, and control flow. This eliminates logical hallucinations. The pipeline combines a concept-first synthesis strategy—extracting
...
To contextualize the effectiveness of Dual Agreement, we compare it to alternative verification strategies on the same 5k validation set. As shown in Table~\ref{tab:verification_comparison}, Dual Agreement achieves 92\% alignment with ground truth, substantially outperforming single execution (67\% accuracy, brittle to flaky tests), while remaining practical at scale unlike formal verification (computationally prohibitive for 85k problems). Compared to LLM-as-Judge, our approach is deterministic (execution results are reproducible) and grounded in operational semantics rather than model intuition. Importantly, while CodeT~\cite{chen2022codetcodegenerationgenerated} uses Dual Agreement to verify \textit{code solutions}, we extend this by generating CoT rationales from execution traces, thereby verifying the \textit{reasoning process itself}. Figure~\ref{fig:consensus_heatmap} and Figure~\ref{fig:consensus_accuracy} provide detailed breakdowns of consensus score scaling and ground-truth alignment across different parameter settings.

\begin{table}[h!]
\centering
\caption{Comparison of verification methods on 5k validation set. Dual Agreement achieves high accuracy while remaining scalable.}
...
To understand how different verification methods affect downstream model performance, we conducted an ablation study where we fine-tuned models on data generated with different verification approaches. Using the same 25k problem set, we generated training data under six conditions: (1) \textbf{Our Approach (Dual Agreement + Trace)}: Full pipeline with Dual Agreement (5 solutions, 10 tests) and trace-grounded rationales, (2) \textbf{Dual Agreement (Small Scale)}: Reduced candidates (2 solutions, 3 tests) with trace rationales, (3) \textbf{LLM-as-Judge}: Solutions verified by prompting an LLM to judge correctness, rationales generated by LLM explanation, (4) \textbf{No Rationale (Q&A Only)}: Dual Agreement verification but no CoT—only question-answer pairs, (5) \textbf{Few-Shot Baseline}: No fine-tuning, using few-shot prompting with 3 examples, and (6) \textbf{Base Model}: No fine-tuning or few-shot prompting.

Table~\ref{tab:verification_ablation} presents the results. Our full approach achieves the highest performance across all benchmarks, with the \texttt{Qwen2.5-Coder-7B} model reaching 68.2\% on LiveCodeBench and 61.9\% on CruxEval Input. The small-scale Dual Agreement variant shows degraded performance (64.1\% LiveCodeBench, -4.1 points), confirming that sufficient candidate diversity is critical for verification quality. The LLM-as-Judge approach performs substantially worse (59.3\% LiveCodeBench, -8.9 points), demonstrating that execution-grounded verification is essential for high-quality training data. Notably, removing rationales entirely (Q&A Only) causes a severe performance drop (52.7\% LiveCodeBench, -15.5 points), validating that trace-grounded reasoning chains provide crucial learning signal beyond input-output mappings. The few-shot baseline (48.1\%) performs only marginally better than the base model (46.3\%), highlighting the value of fine-tuning on verified data.
...
\midrule
Base Model (No FT) & 46.3 & 40.6 & 39.2 & -- \\
Few-Shot (3 examples) & 48.1 & 42.1 & 40.5 & +1.8 \\
\midrule
Q&A Only (No CoT) & 52.7 & 48.3 & 44.6 & +6.4 \\
LLM-as-Judge + LLM CoT & 59.3 & 52.8 & 50.1 & +13.0 \\
Dual Agree. (Small: 2S/3T) & 64.1 & 55.4 & 54.2 & +17.8 \\
\midrule
\textbf{Our Approach (5S/10T + Trace)} & \textbf{68.2} & \textbf{57.3} & \textbf{61.9} & \textbf{+21.9} \\
\bottomrule
\end{tabular}
}
\end{table}
