...
%% article.
\begin{abstract}
  Chain-of-Thought (CoT) fine-tuning teaches language models to reason through step-by-step traces, but generating such training data for code faces a critical challenge: the inability to verify the logical integrity of reasoning steps, leading to models trained on plausible but incorrect "hallucinated" logic. Existing methods validate only final program outputs—leaving intermediate reasoning steps unverified and unsound. We introduce the first system to achieve \textbf{rationale-step verification} at scale by generating CoT rationales directly from program execution traces, making them \textbf{correct by construction}. Each reasoning step is verifiable against the trace: variable values match recorded runtime states, state transitions reflect actual semantic operations, and control flow describes executed branches. Our concept-first synthesis strategy extracts problems from technical literature, ensuring diverse algorithmic coverage. We systematically generate 54,000 high-fidelity, bi-directional rationales that teach models to reason both forward (input$\rightarrow$output) and backward (output$\rightarrow$input), capabilities essential for debugging and program comprehension. Models fine-tuned on our trace-grounded data achieve substantial improvements: \textbf{+14.4 points} on CruxEval-Input and \textbf{+21.9 points} on LiveCodeBench-Exec, demonstrating that verification quality directly determines reasoning capabilities. The complete synthesis pipeline will be open-sourced.
\end{abstract>
...
To address these reasoning limitations, the research community has embraced Chain-of-Thought (CoT) fine-tuning, where models learn from datasets containing not just code, but step-by-step reasoning traces explaining how programs execute. This approach treats reasoning as an explicit process that can be taught through examples. However, generating such training data for code faces a fundamental challenge: \textit{verification}. From a programming languages perspective, this is analogous to the classic verification problem—how do we guarantee that natural language reasoning about code is semantically faithful to the program's actual runtime behavior?

Current methods for generating CoT data are often \textit{unsound}. A powerful "teacher" model generates the rationale, but these reasoning steps are not tethered to a program's actual execution trace. The result is a dataset of "logical hallucinations"—plausible but factually incorrect accounts of the program's logic.
...
\centering
\small
\begin{tabular}{@{}p{0.31\textwidth}@{\hspace{0.02\textwidth}}p{0.31\textwidth}@{\hspace{0.02\textwidth}}p{0.31\textwidth}@{}}
\toprule
\textbf{Code} & \textbf{LLM-Generated (Hallucinated)} & \textbf{Ours (Trace-Grounded)} \\
\midrule
...
\end{lstlisting}
&
\colorbox{errorred!10}{\parbox{0.95\linewidth}{\scriptsize
\textbf{Step 1:} Initialize count=5\\
\textbf{Step 2:} Loop \textcolor{errorred}{\textbf{increments}} count each iteration\\
\textbf{Step 3:} Continue until count=10\\
\textbf{Step 4:} Return 10

\vspace{0.1cm}
\textcolor{errorred}{\textbf{\xmark\ WRONG}}: Claims increment!
}}
&
\colorbox{correctgreen!10}{\parbox{0.95\linewidth}{\scriptsize
\textbf{Trace:}\\
\ttfamily\tiny
\begin{tabular}{@{}l@{}}
count = 5\\
...
\textbf{Step 1:} Initialize count=5\\
\textbf{Step 2:} Loop \textcolor{correctgreen}{\textbf{decrements}} count\\
\textbf{Step 3:} Continue until count=0\\
\textbf{Step 4:} Return 0

\vspace{0.05cm}
\textcolor{correctgreen}{\textbf{\cmark\ VERIFIED}}
}}
\\
\bottomrule
\end{tabular}
\caption{LLM-generated CoT hallucinates (claims increment), while our trace-grounded approach is verifiable.}
