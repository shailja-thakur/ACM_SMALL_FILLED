...
\begin{abstract}
  Chain-of-Thought (CoT) fine-tuning for code suffers from a critical flaw: the reasoning steps in synthetic
     ...
     Existing execution-aware methods perform only outcome verification (validating final outputs), while formal
     verification is computationally prohibitive. We solve this by introducing a pipeline that achieves
     \textbf{rationale-step verification}—validating each reasoning step in the CoT chain—at scale. Our approach
     generates rationales by directly translating program execution traces into natural language, making them correct
     by construction and eliminating logical hallucinations. The pipeline combines a concept-first synthesis strategy—extracting
...
Existing execution-aware methods perform only \textit{outcome verification}—validating the final program output against expected results (e.g., HumanEval~\cite{chen2021evaluating}, SemCoder~\cite{ding2024semcodertrainingcodelanguage}). This validates whether the answer is correct, but provides no guarantee about the logical integrity of intermediate reasoning steps. In contrast, \textit{formal verification} using mathematical proofs (e.g., symbolic execution, SMT solvers) can guarantee correctness but is computationally prohibitive at scale. \textbf{A critical gap exists:} there is no practical method for \textit{rationale-step verification}—validating each reasoning step in the CoT chain—that scales to large-scale training data generation. Additionally, existing pipelines lack fine-grained control over the complexity and diversity of synthetic data.
...
\begin{enumerate}
    \item \textbf{An Execution-Grounded Synthesis Pipeline} that achieves rationale-step verification at scale, generating factually correct CoT data by directly translating execution traces into natural language.
