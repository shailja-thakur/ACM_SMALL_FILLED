...
\textbf{Results and Analysis.} Table~\ref{tab:verification_ablation} presents the results. Our full approach achieves the highest performance across all benchmarks, with \texttt{Qwen2.5-Coder-7B} reaching \textbf{68.2\%} on LiveCodeBench, \textbf{59.7\%} on CruxEval Output, and \textbf{61.9\%} on CruxEval Input. Ablating each component reveals their individual contributions: our minimal execution variant (64.1\%, -4.1 points) demonstrates that verification quality degrades without sufficient candidate diversity, confirming the value of Dual Agreement's statistical consensus for filtering edge-case failures; the LLM-Generated CoT approach performs substantially worse (59.3\%, -8.9 points), validating that execution grounding is essential as LLM-generated explanations lack the factual accuracy guaranteed by trace translation, causing models to learn superficial imitation rather than verifiable execution semantics; removing rationales entirely (Q\&A Only: 52.7\%, -15.5 points) causes severe performance degradation, confirming that trace-grounded reasoning chains provide crucial learning signal beyond input-output mappings. The few-shot baseline (48.1\%, 47.0\%, 48.8\%) performs only marginally better than the base model (46.3\%, 45.3\%, 47.5\%), highlighting the limited value of prompting alone and reinforcing the necessity of fine-tuning on verified data.
