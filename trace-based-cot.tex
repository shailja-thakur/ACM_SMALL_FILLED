...
This is the core of Stage C, where we use the sanitized trace as the factual basis for generating reasoning chains. For forward CoT, we prompt an LLM with the instruction, function code, forward question, and sanitized trace, instructing it to narrate the trace step-by-step, translating how the input transforms into the output. For backward CoT, we prompt the model to perform deductive reasoning: given the final output state in the trace, explain how this output could only have been reached from the initial input. Critically, the model is \textit{not generating an explanation from scratch}; it is \textit{translating} a factual trace into natural language. This eliminates hallucination: variable values, control flow, and return values are all factual records from runtime execution, making every claim in the rationale verifiable against the trace.
