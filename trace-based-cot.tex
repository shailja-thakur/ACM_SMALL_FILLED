...
\begin{figure}[t]
\centering
\begin{tcolorbox}[enhanced, colback=errorred!8, colframe=errorred!80, title=\textbf{LLM-Generated CoT (Contains Hallucination)}, fonttitle=\bfseries, coltitle=white, colbacktitle=errorred!90, boxrule=0.8pt]
\small
\textbf{Code:}
...
\end{lstlisting}
\vspace{-0.1cm}
\textbf{LLM-Generated Explanation:}\\
\textit{"Starting with count=5, the loop} \textcolor{errorred}{\textbf{increments}} \textit{count until reaching 10, then returns 10."}

\vspace{0.1cm}
\textcolor{errorred}{\textbf{✗ HALLUCINATION}}: Claims increment when code decrements!
\end{tcolorbox}

\vspace{0.25cm}

\begin{tcolorbox}[enhanced, colback=correctgreen!8, colframe=correctgreen!80, title=\textbf{Our Trace-Grounded CoT (Verifiable)}, fonttitle=\bfseries, coltitle=white, colbacktitle=correctgreen!90, boxrule=0.8pt]
\small
\textbf{Execution Trace:}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\scriptsize, frame=none, backgroundcolor=\color{gray!10}]
Line 2: count = 5
...
...
Line 5: return count  ->  return: 0
\end{lstlisting}
\vspace{-0.1cm}
\textbf{Our Generated Explanation:}\\
\textit{"Starting with count=5, the loop} \textcolor{correctgreen}{\textbf{decrements}} \textit{count (5→4→3→2→1→0) until reaching 0, then returns 0."}

\vspace{0.1cm}
\textcolor{correctgreen}{\textbf{✓ CORRECT}}: Variable values, state transitions, and control flow match trace!
\end{tcolorbox}

\caption{Comparison of LLM-generated vs trace-grounded CoT. Traditional CoT generation produces plausible but incorrect reasoning (top), while our approach translates execution traces directly into natural language, making rationales verifiable against actual program behavior (bottom).}
