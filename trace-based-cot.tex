...
\textbf{Stage 2: Signature Generation.} Before generating any code or tests, we prompt the LLM to analyze the instruction and produce a formal signature skeleton specifying: (1) implementation type (standalone function vs. class), (2) function signature with parameter types and return type, or (3) class signature with constructor and methods. The signature prompt uses a strict formatting template enforced via few-shot examples. For instance, a function signature follows the format:
\begin{verbatim}
Function: solution(freq_list: list[tuple[str, int]])
  -> dict[str, str]
\end{verbatim}
while a class signature follows:
\begin{verbatim}
Class: HuffmanTree; __init__(self, freq_list:
  list[tuple[str, int]]) -> None;
  build_tree(self) -> tuple;
  get_encoding(self) -> dict[str, str]
\end{verbatim}
...
\textbf{Stage 5: Test Generation.} We generate unit tests (3 test suites per instruction, each with up to 10 test functions) using a meticulously engineered prompt enforcing: (1) \textit{Standalone test functions:} Each test is a top-level function, not a class method, simplifying parsing. (2) \textit{One assert per function:} Each test contains exactly one assert statement, critical for granular failure diagnostics in Stage B. (3) \textit{Direct function calls in asserts:} The assert must directly call the function with inline arguments, as variable assignments complicate trace generation. (4) \textit{Concrete expected outputs:} The model must calculate exact expected values, not placeholders. For example, a correct test follows this format:
\begin{verbatim}
def test_basic_functionality():
    # Test basic scenario
    assert solution([1, 2, 3], 2) == [1]
\end{verbatim}
while the following prohibited format would complicate trace extraction:
\begin{verbatim}
def test_basic_functionality_wrong():
    # Variable outside assert (prohibited)
    lst = [1, 2, 3]
    assert solution(lst, 2) == [1]
\end{verbatim}
...
\For{each solution $s_i \in S$}
    \State $\textit{pattern}_i \gets M[i, :]$ \Comment{Test pass/fail pattern}
    \State Add $s_i$ to cluster with matching pattern in \textit{clusters}
\EndFor
\For{each cluster $C_i \in \textit{clusters}$}
    \State $T_p(C_i) \gets \{t_j : \forall s \in C_i, M[s, j] = 1\}$ \Comment{Commonly passed tests}
    \State $\textit{Score}(C_i) \gets |C_i| \times |T_p(C_i)|$
\EndFor
\State $C^* \gets \arg\max_{C_i} \textit{Score}(C_i)$ \Comment{Highest-scoring cluster}
