...
This challenge is especially acute in code reasoning, where small logical errors cascade into flawed understanding. Consider a model trained on a rationale claiming "variable \texttt{count} is incremented"—when it's actually decremented. Such errors propagate during training, teaching plausible but incorrect reasoning patterns. This lack of guaranteed soundness is a critical barrier for high-stakes applications. For \textbf{debugging}, a rationale that misrepresents variable state is actively misleading. For \textbf{iterative self-refinement}, feeding a model its own flawed reasoning causes compounding errors. For \textbf{agentic workflows}, unsound understanding can derail entire task sequences. Figure~\ref{fig:motivation} illustrates this problem concretely.
...
% \definecolor{processcolor}{RGB}{230,240,255}
% \definecolor{finalcolor}{RGB}{230,255,230}
% % Assuming 'errorred' and 'correctgreen' are defined elsewhere
% % \definecolor{errorred}{RGB}{...}
% % \definecolor{correctgreen}{RGB}{...}
...
% &
% % --- Column 2: LLM-Generated CoT ---
% \colorbox{errorred!8}{\parbox[t]{0.96\linewidth}{\scriptsize
...
% }}
% &
% % --- Column 3: Execution Trace ---
% \colorbox{gray!15}{\parbox[t]{0.96\linewidth}{\strut\ttfamily\scriptsize
% \colorbox{initcolor}{\textbf{Initialization}}\nLine 2: left = 0\nLine 3: right = 4\\[0.1cm]
% \colorbox{processcolor}{\textbf{Loop Iteration 1}}\nLine 5: mid = 2\nLine 6: arr[2]=5, arr[3]=4\nLine 9: right: 4 $\rightarrow$ 2\\[0.1cm]
% \colorbox{processcolor}{\textbf{Loop Iteration 2}}\nLine 5: mid = 1\nLine 6: arr[1]=3, arr[2]=5\nLine 7: left: 0 $\rightarrow$ 2\\[0.1cm]
% \colorbox{finalcolor}{\textbf{Return}}\nLine 10: return 2
% }}
% &
% % --- Column 4: Our CoT ---
% \colorbox{correctgreen!8}{\parbox[t]{0.96\linewidth}{\scriptsize
...
% &
% % --- Column 3: Execution Trace ---
% \colorbox{gray!15}{\parbox[t]{0.96\linewidth}{\ttfamily\tiny
...
\textbf{A critical gap exists:} while prior work validates final program outputs~\cite{chen2021evaluating,ding2024semcodertrainingcodelanguage} or relies on computationally prohibitive formal methods, there is no practical approach for validating each step in the reasoning chain—what we term \textit{rationale-step verification}—that scales to large-scale training data generation. Additionally, existing pipelines lack fine-grained control over problem complexity and diversity.
...
In this work, we bridge these gaps by introducing a new paradigm for data synthesis that achieves \textit{rationale-step verification} at scale. We have developed a complete synthesis pipeline that generates CoT rationales by directly translating program execution traces into natural language. Specifically, we instrument programs to capture their dynamic execution state—recording variable values, control flow decisions, and state transitions at each step. This trace serves as an immutable ground truth from which rationales are derived, ensuring each reasoning step can be verified against actual runtime behavior. This makes rationales \textbf{correct by construction}, eliminating the logical hallucinations that plague other methods.
...
\begin{enumerate}
    \item \textbf{Chain of Thought Step Verification at Scale}: We introduce the first execution-grounded synthesis pipeline that achieves rationale-step verification at scale. 

    \item \textbf{Concept-First Curriculum Synthesis}: A novel curriculum-driven generation approach providing fine-grained control over problem complexity and algorithmic diversity.

    \item \textbf{Bi-Directional Reasoning Dataset}: The first large-scale dataset (25,000 samples) systematically teaching both forward (input→output) and backward (output→input) reasoning with trace-grounded verification.

    \item \textbf{Comprehensive Empirical Validation}: Through systematic ablations, we demonstrate that verification quality directly determines model performance.

    \item \textbf{Open-Source Pipeline}: Complete synthesis infrastructure including concept extraction, hierarchical generation, Dual Agreement verification, and trace-to-rationale translation for verified code reasoning research.
\end{enumerate}
...
Our work builds upon several interconnected research areas in code reasoning, execution-based program understanding, and synthetic data generation. Chain-of-Thought (CoT) prompting~\cite{wei2022chain} has proven effective for improving reasoning, but its extension to code faces unique verification challenges. While methods like Program-of-Thought (PoT)~\cite{chen2022program} generate executable snippets and CodeI/O~\cite{li2025codeio} transforms code into I/O prediction tasks, neither systematically generates trace-grounded rationales at scale.

\textbf{Verification Approaches in Code Generation.} The term "verification" appears frequently in code generation literature but often refers to different concepts. We identify three distinct approaches: (1) \textbf{Outcome Verification} validates only the final program output against expected results. This is the most common approach, used by benchmarks like HumanEval~\cite{chen2021evaluating} and methods like SemCoder~\cite{ding2024semcodertrainingcodelanguage} and rStar-Coder~\cite{liu2025rstarcoderscalingcompetitivecode}. While effective, it provides no guarantee about intermediate reasoning steps. (2) \textbf{Formal Verification} uses mathematical proofs (e.g., SMT solvers) to guarantee program correctness. While sound, this approach is computationally heavyweight and impractical for large-scale LLM training. (3) \textbf{Rationale-Step Verification} (our contribution) validates each reasoning step in the CoT chain. By generating rationales directly from a program's execution trace—a factual record capturing variable values, state transitions, and control flow—we verify that each step correctly describes the variable values, state changes, and control flow paths. This verifies the logical integrity of the entire reasoning process, not just the final outcome.
...
Our data synthesis pipeline (Figure~\ref{fig:system_overview}) is a multi-stage process designed to generate high-fidelity, bi-directional CoT data where every reasoning step is anchored in ground-truth program execution. This ensures that the resulting dataset is correct by construction, eliminating the logical hallucinations common in purely model-generated rationales.
...
\textbf{Step 2: Signature Generation.} Before generating any code or tests, we prompt the LLM to analyze the instruction and produce a formal signature skeleton specifying: (1) implementation type (standalone function vs. class), (2) function signature with parameter types and return type, or (3) class signature with constructor and methods. The signature prompt uses a strict formatting template enforced via few-shot examples, as shown in Figure~\ref{fig:signature-formats}. From each signature, we extract and store metadata including function names, parameter names and types, and crucially, return variable names with their types (e.g., \texttt{encoding: dict[str, str]}). This metadata serves as ground truth for validating consistency across code, tests, and generated CoT rationales. This signature-first design is deliberate: once input/output types and function names are fixed, subsequent code generation cannot hallucinate different types or names, eliminating import failures and ensuring test-code alignment.
...
% \textbf{Mathematical Formulation of Noise Reduction.} 

We can formalize why this scoring creates clear separation between correct and incorrect clusters. Let $C_{correct}$ be the cluster of correct solutions and $C_{error}$ be any cluster of incorrect solutions. For the correct cluster:
\[
...
\begin{enumerate}
    ...
    \subsubsection{Hyperparameter Selection for Data Generation}
    To determine the optimal number of candidate solutions and unit tests per problem, we conducted systematic experiments with varying configurations. We evaluated different numbers of candidate solutions (5, 10, 15, 20) and unit test cases (5, 10, 20, 30, 40, 50) per instruction. Based on our ablation studies balancing quality and computational cost, we selected 5 candidate solutions and 30 unit tests per problem. The 30 unit tests were generated in 3 sets of 10 unit tests each, as described in Section 3.1, ensuring comprehensive coverage of edge cases and core functionality while maintaining diversity through multiple independent generation rounds.
...
\subsection{Validating Dual Agreement}
...
% We conducted a validation study on 5,000 programming problems for which we had ground-truth correct solutions. For each problem, we systematically varied two parameters: the number of candidate solutions (5, 10, 15, 20) and the number of test cases (5, 10, 20, 30, 40, 50), measuring both consensus scores and alignment with ground truth. This allowed us to quantify how the dual dimensions of solution agreement and test coverage affect verification reliability.

Before presenting the main experimental results, we validate the effectiveness of our Dual Agreement verification approach (Stage B) through a controlled study on 5,000 programming problems with ground-truth correct solutions. Using the hyperparameter configurations described in Section 5.1.1, we measured both consensus scores and alignment with ground truth to quantify how the dual dimensions of solution agreement and test coverage affect verification reliability.

\subsubsection{Consensus Score Scaling with Candidates}
Figure~\ref{fig:consensus_heatmap} shows a heatmap of the percentage of problems achieving high consensus scores (Score $> \tau$ threshold) as a function of the number of solutions (x-axis) and test cases (y-axis). The heatmap reveals a clear pattern: the bottom-right region (high solutions, high tests) shows consistently high consensus rates (70-90\% of problems), while the upper-left region (few solutions, few tests) shows near-zero consensus rates. This validates our hypothesis that \textit{both} multiple diverse solutions and comprehensive test suites are necessary for reliable verification. Notably, increasing solutions from 5 to 20 yields a 3x increase in problems with high consensus scores when test coverage is adequate ($\geq 20$ tests). This empirically confirms the theoretical exponential decay property: more candidates exponentially reduce the probability of false consensus.

\begin{figure}[t!]
    \centering
    \includegraphics[width=0.75\columnwidth]{code_test_consensus.png}
    \caption{Heatmap showing percentage of problems achieving high consensus scores as a function of number of candidate solutions (x-axis) and test cases (y-axis). The bottom-right region (many solutions, many tests) achieves 70-90\% consensus, while the top-left (few solutions, few tests) shows near-zero consensus, validating that both dimensions are necessary for reliable verification.}
...
% \begin{figure}[t!]
%     \centering
%     \includegraphics[width=0.85\columnwidth]{consensus_rate.png}
%     \caption{Consensus rate versus alignment with ground truth. As the consensus rate increases from 20\% to 90\%, alignment with ground truth increases monotonically from 30\% to 95\% (95\% confidence intervals shown). This validates that Dual Agreement's scoring mechanism effectively identifies correct solutions.}
%     \label{fig:consensus_accuracy}
% \end{figure}
...
\begin{table*}[t!]
    ...
    \begin{tabular}{C{2.5cm}|l|c|c|cc|cc}
    \hline
    \multirow{2}{*}{\textbf{Model}} & \multirow{2}{*}{\textbf{CoT Direc.}} & \multirow{2}{*}{\textbf{Data Subset}} &
    \textbf{LCB-Exec} &
    ...
    % \hline
    \parbox{2.5cm}{\centering Granite-3.3-8B-base} & Base (default) & N/A & 18.3 & 15.5 & 25.3 & 14.3 & 28.4 \\
    \hline
    \multirow{6}{*}{\parbox{2.5cm}{\centering Granite-3.3-8B (FT)}} & \multirow{3}{*}{Fwd} & 18k & 43.5$_{\textcolor{blue}{(+25.2)}}$ & 36.1$_{\textcolor{blue}{(+20.6)}}$ & 58.2$_{\textcolor{blue}{(+32.9)}}$ & 35.8$_{\textcolor{blue}{(+21.5)}}$ & 57.9$_{\textcolor{blue}{(+29.5)}}$ \\
     & & 25k (best) & \textbf{44.9}$_{\textcolor{blue}{(+26.6)}}$ & \textbf{42.7}$_{\textcolor{blue}{(+27.2)}}$ & \textbf{64.7}$_{\textcolor{blue}{(+39.4)}}$ & \textbf{40.2}$_{\textcolor{blue}{(+25.9)}}$ &
    \textbf{63.5}$_{\textcolor{blue}{(+35.1)}}$ \\
     & & 54k & 34.1$_{\textcolor{blue}{(+15.8)}}$ & 28.9$_{\textcolor{blue}{(+13.4)}}$ & 55.2$_{\textcolor{blue}{(+29.9)}}$ & 28.8$_{\textcolor{blue}{(+14.5)}}$ & 54.9$_{\textcolor{blue}{(+26.5)}}$ \\
    \cline{2-8}
     & Fwd & 25k & \cellcolor{highlightgray}{\textbf{44.9}$_{\textcolor{blue}{\textbf{(+26.6)}}}$} & 42.7$_{\textcolor{blue}{(+27.2)}}$ & 64.7$_{\textcolor{blue}{(+39.4)}}$ & 40.2$_{\textcolor{blue}{(+25.9)}}$ & 63.5$_{\textcolor{blue}{(+35.1)}}$ \\
     & Bwd & 25k & 35.4$_{\textcolor{blue}{(+17.1)}}$ & 39.3$_{\textcolor{blue}{(+23.8)}}$ & 61.3$_{\textcolor{blue}{(+36.0)}}$ & 41.5$_{\textcolor{blue}{(+27.2)}}$ & 64.8$_{\textcolor{blue}{(+36.4)}}$ \\
     & Bi-directional & 25k & 44.3$_{\textcolor{blue}{(+26.0)}}$ & \cellcolor{highlightgray}{\textbf{45.7}$_{\textcolor{blue}{\textbf{(+30.2)}}}$} & \cellcolor{highlightgray}{\textbf{67.4}$_{\textcolor{blue}{\textbf{(+42.1)}}}$} & \cellcolor{highlightgray}{\textbf{42.1}$_{\textcolor{blue}{\textbf{(+27.8)}}}$} & \cellcolor{highlightgray}{\textbf{65.2}$_{\textcolor{blue}{\textbf{(+36.8)}}}$} \\
    \hline
    \hline
    % \multicolumn{9}{c}{\textit{Qwen2.5-7B Model Family}} \\
    % \hline
    \parbox{2.5cm}{\centering Qwen2.5-Coder-7B} & Base (default) & N/A & 46.3 & 45.3 & 52.12 & 47.5 & 49 \\
    \hline
    \multirow{6}{*}{\parbox{2.5cm}{\centering Qwen2.5-Coder-7B (FT)}} & \multirow{3}{*}{Fwd} & 18k & 66.9$_{\textcolor{blue}{(+20.6)}}$ & 58.4$_{\textcolor{blue}{(+13.1)}}$ & 75.5$_{\textcolor{blue}{(+23.4)}}$ &
    57.2$_{\textcolor{blue}{(+9.7)}}$ & 69.4$_{\textcolor{blue}{(+20.4)}}$ \\
     & & 25k & \textbf{67.0}$_{\textcolor{blue}{(+20.7)}}$ & 57.5$_{\textcolor{blue}{(+12.2)}}$ & 73.9$_{\textcolor{blue}{(+21.8)}}$ & 59.8$_{\textcolor{blue}{(+12.3)}}$ & 67.5$_{\textcolor{blue}{(+18.5)}}$ \\
     & & 54k (best) & 66.5$_{\textcolor{blue}{(+20.2)}}$ & \textbf{58.6}$_{\textcolor{blue}{(+13.3)}}$ & \textbf{76.0}$_{\textcolor{blue}{(+23.9)}}$ & \textbf{60.5}$_{\textcolor{blue}{(+13.0)}}$ & \textbf{68.3}$_{\textcolor{blue}{(+19.3)}}$ \\
    \cline{2-8}
     & Fwd & 25k & 67.0$_{\textcolor{blue}{(+20.7)}}$ & 57.5$_{\textcolor{blue}{(+12.2)}}$ & 73.9$_{\textcolor{blue}{(+21.8)}}$ & 59.8$_{\textcolor{blue}{(+12.3)}}$ & 67.5$_{\textcolor{blue}{(+18.5)}}$ \\
     & Bwd & 25k & 57.5$_{\textcolor{blue}{(+11.2)}}$ & 50.4$_{\textcolor{blue}{(+5.1)}}$ & 69.8$_{\textcolor{blue}{(+17.7)}}$ & 61.2$_{\textcolor{blue}{(+13.7)}}$ & 69.1$_{\textcolor{blue}{(+20.1)}}$ \\
     & Bi-directional & 25k & \cellcolor{highlightgray}{\textbf{68.2}$_{\textcolor{blue}{\textbf{(+21.9)}}}$} & \cellcolor{highlightgray}{\textbf{59.7}$_{\textcolor{blue}{\textbf{(+14.4)}}}$} & \cellcolor{highlightgray}{\textbf{75.4}$_{\textcolor{blue}{\textbf{(+23.3)}}}$} & \cellcolor{highlightgray}{\textbf{61.9}$_{\textcolor{blue}{\textbf{(+14.4)}}}$} & \cellcolor{highlightgray}{\textbf{70.2}$_{\textcolor{blue}{\textbf{(+21.2)}}}$} \\
...
\textbf{Does the method used to generate and verify chain-of-thought rationales fundamentally impact model reasoning capabilities, or are all CoT approaches equivalent?} To answer this question, we systematically evaluate how the \textit{source} and \textit{validation} of rationales affect downstream model performance. We are not merely comparing verification strategies, but rather fundamentally different approaches to CoT generation: execution-grounded rationales (guaranteed correct by construction), LLM-generated rationales (potentially hallucinated), and no rationales at all.

\textbf{Experimental Design.} Using the same 25k problem set from our data curation ablation, we generated training data under six distinct conditions and fine-tuned \texttt{Qwen2.5-Coder-7B} models for comparison:

\begin{enumerate}[nosep, leftmargin=*]
    \item \textbf{Baseline Approaches:} (1) \textbf{Base Model}: No fine-tuning; (2) \textbf{Few-Shot}: 3-shot prompting with example problems, no fine-tuning.
    \item \textbf{Alternative CoT Strategies:} (3) \textbf{Q\&A Only}: Fine-tuned on question-answer pairs with no CoT rationales; (4) \textbf{LLM-Generated CoT}: Fine-tuned on CoT rationales generated purely by an LLM without execution grounding.
    \item \textbf{Our Execution-Grounded Approach:} (5) \textbf{Minimal Execution}: Our trace-grounded CoT using minimal verification (1 solution, 1 test); (6) \textbf{Full Approach}: Our complete pipeline with Dual Agreement verification (10 solutions, 25 tests) and trace-grounded rationales.
\end{enumerate}

\textbf{Results and Analysis.} Table~\ref{tab:verification_ablation} presents the complete results. The ablation reveals a clear hierarchy of components, with three key insights. \textit{First}, execution grounding is the dominant factor: LLM-generated CoT, despite providing step-by-step rationales, fundamentally fails because it trains models on plausible but unverified reasoning patterns. This validates our core thesis that the \textit{source} of rationales—whether grounded in actual execution or generated from statistical patterns—determines whether models internalize verifiable semantics or learn superficial imitation. \textit{Second}, verification quality compounds this effect: even when using execution-grounded rationales, insufficient candidate diversity degrades performance, as single solution-test pairs cannot filter edge-case failures. This confirms that statistical consensus through Dual Agreement is not merely a scalability mechanism but a necessary correctness guarantee. \textit{Third}, the step-by-step reasoning itself is essential: removing CoT rationales entirely causes the largest performance drop, demonstrating that models require explicit traces of \textit{how} outputs arise from inputs, not just input-output correspondences. Prompting alone provides negligible improvement, underscoring that these reasoning capabilities must be internalized through fine-tuning on verified data rather than retrieved at inference time.

\textbf{Conclusion.} This evaluation demonstrates that execution-grounded rationale generation represents a fundamentally different paradigm from LLM-generated explanations. The method is not a technical implementation detail—it determines whether models learn verifiable reasoning or statistical mimicry of reasoning patterns. For code, where correctness is non-negotiable, only execution-grounded approaches provide the necessary foundation for trustworthy reasoning capabilities.
...
% \begin{tabular}{lcccc}
% \toprule
% \textbf{Verification Approach} & \textbf{LiveCodeBench} & \textbf{CruxEval-O} & \textbf{CruxEval-I} & HumanEval \\
% \midrule
% Base Model (No FT) & 46.3 & 45.3 & 47.5 & - \\
% Few-Shot (3 examples) & 48.1 & 47.0 & 48.8 & - \\
% \midrule
% Q\&A Only (No CoT) & 52.7 & 50.2 & 51.3 & - \\
% LLM-as-Judge + LLM CoT & 59.3 & 54.1 & 55.7 & - \\
% % Dual Agree. (Small: 1S/1T) & 64.1 & 56.8 & 58.4 & - \\
% Dual Agree. (Small: 1S/1T) & - & - & - & - \\
% \midrule
% \textbf{Our Approach (10S/25T + Trace)} & \textbf{68.2} & \textbf{59.7} & \textbf{61.9} & - \\
% \bottomrule
% \end{tabular}
% }
% \end{table}
...
\toprule
\textbf{Verification Approach} & \textbf{LiveCodeBench} & \textbf{CruxEval-O} & \textbf{CruxEval-I} & \textbf{HumanEval} \\
\midrule
Base Model (No FT) & 46.3 & 45.3 & 47.5 & 62.0 \\
Few-Shot (3 examples) & 48.1 & 47.0 & 48.8 & 63.5 \\
\midrule
Q\&A Only (No CoT) & 52.7 & 50.2 & 51.3 & 69.0 \\
LLM-as-Judge + LLM CoT & 59.3 & 54.1 & 55.7 & 75.5 \\
% Dual Agree. (Small: 1S/1T) & 64.1 & 56.8 & 58.4 & 78.5 \\
Dual Agree. (Small: 1S/1T) & - & - & - & - \\
\midrule
\textbf{Our Approach (10S/25T + Trace)} & \textbf{68.2} & \textbf{59.7} & \textbf{61.9} & \textbf{81.5} \\
\bottomrule
\end{tabular}
}
\end{table}
...
% The comprehensive results of all ablation experiments are presented in Table \ref{tab:comprehensive-results}. Our bi-directional training achieves the strongest performance across all benchmarks, with Qwen2.5-Coder-7B reaching 68.2\% on LiveCodeBench (+21.9 points) and Granite-3.3-8B achieving 44.9\% (+26.6 points), demonstrating consistent improvements in code generation capabilities. Additionally, Table \ref{tab:verification_ablation} shows that our full approach achieves 81.5\% on HumanEval, significantly outperforming the baseline (62.0\%) and alternative verification methods.

% The comprehensive results of all ablation experiments are presented in Table \ref{tab:comprehensive-results}.
