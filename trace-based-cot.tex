...
\begin{abstract}
  Chain-of-Thought (CoT) fine-tuning for code suffers from a critical flaw: the reasoning steps in synthetic
     ...
     Existing methods validate only the final program output, not the logical integrity of intermediate reasoning
     steps. We solve this by introducing a pipeline that achieves \textbf{rationale-step verification}—validating
     each reasoning step in the CoT chain—at scale. Our approach
