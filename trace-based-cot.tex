...
%% article.
\begin{abstract}
  Chain-of-Thought (CoT) fine-tuning for code suffers from a critical flaw: the reasoning steps in synthetic
     datasets are often plausible but unverified "logical hallucinations," training models to replicate flawed logic.
     We solve this by introducing a pipeline that generates \textbf{Trace-Grounded Rationales}: CoT datasets where
     every reasoning step is a verifiable, factual account of a program's actual execution. Our approach generates
     rationales by directly translating execution traces into natural language, making them correct by construction
     and eliminating logical hallucinations. The pipeline combines a concept-first synthesis strategy—extracting
     problems from technical literature spanning diverse algorithmic and mathematical domains—with execution-grounded
     verification at scale. We systematically create 54,000 high-fidelity, bi-directional (input-to-output and
     output-to-input) rationales. Through extensive evaluations, we demonstrate that models trained on our data achieve
     superior reasoning capabilities, boosting pass@1 performance by over \textbf{14 points} on CruxEval and over
     \textbf{21 points} on LiveCodeBench-Exec. The complete data synthesis pipeline will be made available as
     open-source to facilitate further research.
\end{abstract>
...
A central theme in recent work is "verification," but the term is often used ambiguously. To clarify our contribution within this landscape, we propose a taxonomy of verification for code generation data:
\begin{itemize}[nosep]
\item \textbf{Outcome Verification:} Validates only the final program output against expected results (e.g., HumanEval~\cite{chen2021evaluating}, rStar-Coder~\cite{liu2025rstarcoderscalingcompetitivecode}, SemCoder~\cite{ding2024semcodertrainingcodelanguage}). This is the most common approach but offers no guarantee about the logical integrity of intermediate reasoning steps.
\item \textbf{Formal Verification:} Uses mathematical proofs to guarantee program correctness (e.g., symbolic execution, SMT solvers). While sound, this is computationally heavyweight and impractical for large-scale LLM training data generation.
\item \textbf{Rationale-Step Verification:} Validates each reasoning step in the CoT chain (our contribution). By generating rationales directly from a program's operational semantics captured in execution traces, we verify the logical integrity of the entire reasoning process, not just the final outcome.
\end{itemize}
Our work is the first to systematically generate CoT data with rationale-step verification at scale.
     
Recent work has increasingly recognized the value of program execution for creating training data. TRACED~\cite{ding2024traced} introduces execution-aware pre-training that incorporates dynamic program states, while Execution Tuning (E.T.)\citep{armengol2025execution} trains models on execution traces at different granularities. A highly relevant recent work, SemCoder~\cite{ding2024semcodertrainingcodelanguage}, also generates a synthetic dataset for fine-tuning. However, our methodology is fundamentally different in two key aspects.

\textbf{First is the nature of the CoT verification.} Our pipeline performs a \textbf{direct translation} of a program's execution trace into a natural language rationale, making the reasoning process itself verifiable by construction. More precisely, SemCoder generates rationales via an LLM's \textit{explanation} of code behavior, then verifies only that the final predicted output matches an execution result. In contrast, our pipeline performs a direct \textit{narration} of the execution trace into natural language, making each intermediate reasoning step verifiable against the trace log. This fundamental difference—narration vs. generation—is what eliminates logical hallucinations. \textbf{Second is the data generation philosophy.} Our pipeline follows a \textit{``concept-first''} approach, synthesizing problems from abstract concepts extracted from diverse sources like books and documentation. This provides fine-grained control over the complexity and diversity of the resulting data, a key advantage over the \textit{``code-first''} approach used by SemCoder, which is seeded by a fixed set of existing programs. Conceptually closest to our CoT generation step, Jung et al.\cite{jung2025codeexecutiongroundedsupervision} translate execution traces into natural language CoT. However, their work focuses on single-direction reasoning and does not systematically explore the bi-directional synthesis pipeline that we propose.
...
The generation of high-quality synthetic data remains a central challenge. A significant recent contribution is rStar-Coder\cite{liu2025rstarcoderscalingcompetitivecode}, which scales up verified data for competitive programming. Both our work and rStar-Coder leverage execution, but our approaches differ fundamentally. rStar-Coder verifies the \textit{final outcome} of generated code. Our pipeline verifies \textit{every intermediate reasoning step} by generating rationales directly from execution traces. Consequently, rStar-Coder's rationales are LLM-generated explanations (subject to hallucination), while ours are correct by construction. In summary, rStar-Coder excels at creating large-scale datasets of verifiably correct \textit{solutions}, while our work is the first to create datasets of verifiably correct \textit{reasoning processes} in a systematic, bi-directional format.
...
For each high-quality concept, we synthesize a complete problem artifact set through a carefully ordered multi-stage prompting pipeline. The ordering is designed to \textit{constrain the solution space at each step}, preventing type and naming inconsistencies.

\textbf{Stage 1: Instruction Generation.} Given a concept and its description, we prompt the teacher model to generate 6 distinct natural language problem instructions. The prompt is engineered for diversity along three axes: (1) problem domain (mathematics, finance, data processing, algorithms), (2) computational approach (iterative, recursive, dynamic programming, functional), and (3) difficulty (3 "medium" targeting 30-60 LOC, 3 "hard" targeting 50-100+ LOC). The prompt includes explicit anti-overlap guidance to avoid paraphrased versions of the same problem.

\textbf{Stage 2: Signature Generation.} Before generating any code or tests, we prompt the LLM to analyze the instruction and produce a formal signature skeleton specifying: (1) implementation type (standalone function vs. class), (2) function signature with parameter types and return type, or (3) class signature with constructor and methods. The signature prompt uses a strict formatting template enforced via few-shot examples, as shown in Figure~\ref{fig:signature-formats}.

\textbf{Why Signature-First Design Prevents Hallucination.} Generating the signature before code and tests is deliberate: (1) \textit{Type consistency:} Once input/output types are fixed, candidates cannot hallucinate different types. (2) \textit{Naming consistency:} The function/method name is fixed, eliminating import failures during execution. (3) \textit{Parameter alignment:} Tests call the function with correct arity and parameter order, eliminating test-code misalignment.

\textbf{Stage 3: Code Generation.} With the signature locked, we generate 5 candidate solutions per instruction by prompting with both the instruction and signature. The prompt explicitly instructs: (1) strictly adhere to the signature, (2) vary computational approach across implementations, and (3) ensure completeness.

\textbf{Stage 4: Test Scenario Identification.} We prompt the LLM to analyze the problem and produce a concise list of test scenarios (e.g., "Test basic functionality", "Test empty input"). This improves test coverage by encouraging reasoning about requirements before writing code.

\textbf{Stage 5: Test Generation.} We generate unit tests (3 test suites per instruction, each with up to 10 test functions) using a meticulously engineered prompt enforcing: (1) \textit{Standalone test functions:} Each test is a top-level function, not a class method, simplifying parsing. (2) \textit{One assert per function:} Each test contains exactly one assert statement, critical for granular failure diagnostics in Stage B. (3) \textit{Direct function calls in asserts:} The assert must directly call the function with inline arguments, as variable assignments complicate trace generation. (4) \textit{Concrete expected outputs:} The model must calculate exact expected values, not placeholders. Figure~\ref{fig:test-formats} shows the correct format versus the prohibited format.

\textbf{Why This Test Structure Matters for Dual Execution.} The constraints are motivated by Stage B requirements: (1) granular pass/fail signals enable fine-grained agreement clustering, (2) direct calls yield clean traces for Stage C, and (3) standalone functions enable parallel sandboxed execution.
...
\textbf{Best Cluster Selection.} For each task, we rank clusters by score and select the \textbf{single highest-scoring cluster}. We extract one canonical solution from this cluster (selecting the shortest or most readable) and its associated passing tests. Algorithm~\ref{alg:dual_agreement} formalizes this procedure.
...
\textbf{Why Dual Agreement Filters Noise.} This method addresses multiple noise sources: (1) \textit{Incorrect solutions:} A buggy solution typically fails many tests, landing in a low-scoring cluster (low $|T_p|$). (2) \textit{Malformed tests:} A test with wrong expected output causes correct solutions to fail it, reducing $|T_p|$ for the correct cluster, but correct tests pass correct solutions, increasing their $|T_p|$. (3) \textit{Ambiguous instructions:} If an instruction is ambiguous, solutions implement different interpretations, fragmenting into multiple small clusters (low $|C_i|$). The highest-scoring cluster represents the most common interpretation.

\subsubsection{Theoretical Justification: Why Multiple Candidates Matter}
The effectiveness of Dual Agreement relies critically on generating multiple diverse candidates. While a single solution-test pair provides no signal about correctness, multiple independently generated candidates enable statistical consensus detection.

\textbf{Unlikely Collision Property.} The probability that $K$ independently incorrect solutions would \textit{all} pass the same incorrect test suite decreases exponentially with $K$. Formally, if each incorrect solution has probability $p$ of coincidentally passing an incorrect test (due to implementation bugs canceling out test errors), then the probability that all $K$ incorrect solutions pass is $p^K$. For reasonable values (e.g., $p = 0.3$, $K = 5$), this yields $p^K = 0.00243$, making such false consensus highly unlikely. In contrast, $K$ correct solutions will deterministically pass correct tests, yielding $\text{Score}(C) = K \times |T_p|$ with high $|T_p|$.

\textbf{Consensus as Quality Signal.} Large $|C_i|$ (solution agreement) and large $|T_p(C_i)|$ (test coverage) provide two independent signals of correctness. A cluster with $|C_i| = 1$ (single solution) provides no consensus; a cluster with $|T_p| = 1$ (single passing test) provides weak validation. Only clusters with \textit{both} high $|C_i|$ and high $|T_p|$ survive the scoring threshold, filtering out isolated or weakly-validated artifacts.
...
\]
where $n_{valid}$ is the number of valid tests (all pass correct solutions). For an incorrect cluster:
\[
\mathbb{E}[\text{Score}(C_{error})] \leq \mathbb{E}[|C_{error}|] \times n_{valid} \times (1 - \delta)
\]
where $\delta > 0$ represents the fraction of tests that expose the bug. The key insight is that incorrect solutions typically fail at least some tests ($\delta > 0$), creating a score gap. When we have $K$ diverse candidate solutions with $m$ correct among them, the expected score ratio becomes:
\[
\frac{\mathbb{E}[\text{Score}(C_{correct})]}{\mathbb{E}[\text{Score}(C_{error})]} \geq \frac{m \cdot n_{valid}}{(K-m) \cdot n_{valid} \cdot (1-\delta)} = \frac{m}{(K-m)(1-\delta)}
\]
As $K$ increases with sufficient diversity (more independent implementations), $m$ (correct solutions) grows while incorrect solutions fragment into multiple small clusters with different error patterns, each with low $|C_{error}|$. This multiplicative scoring creates exponential separation, allowing correct clusters to dominate even with noisy candidates.

\textbf{Why This Requires Diversity.} Generating diverse candidates (via temperature sampling, prompt variation, and varied computational approaches in Stage A) is essential. If all candidates implement the same algorithm with the same bug, they will cluster together with high $|C_i|$ but fail the same tests, yielding low $|T_p|$. Diversity ensures that correct implementations converge to the same cluster while incorrect ones fragment. The effectiveness of this approach is empirically validated in Section~\ref{sec:dual_agreement_validation}.
...
\subsection{Stage C: Execution-Grounded CoT Generation}
The final stage transforms each verified solution-test pair into a rich, conversational training dataset where every reasoning step is grounded in the program's actual execution. This is not a single-prompt process, but a meticulous, multi-step automated pipeline.
...
\subsubsection{Step 3: Ground-Truth Input/Output Extraction}
Before generating CoT rationales, we establish the precise input and output for each test case. For simple test functions like \texttt{def test\_basic(): assert solution([1, 2], 2) == [1]}, we use regex parsing to extract \texttt{input = [1, 2], 2} and \texttt{expected\_output = [1]}. For complex tests involving setup logic, we prompt an LLM to analyze the test code and extract the effective input/output pair.

\subsubsection{Step 4: Question Generation}
With ground-truth I/O established, we prompt an LLM to generate a pair of natural language questions: (1) \textbf{Forward question:} "Given the input \texttt{[1, 2, 3]} and \texttt{target=2}, what does the function return?", and (2) \textbf{Backward question:} "What input would cause the function to return \texttt{[1]}?" The prompt explicitly instructs the model to phrase questions naturally, adding linguistic diversity to the dataset.

\subsubsection{Step 5: Trace-Grounded Chain-of-Thought Generation}
This is the core of Stage C, where we use the sanitized trace as the factual basis for generating reasoning chains.

\textbf{Forward CoT Generation: Narrating the Trace.} We prompt an LLM with: (1) the instruction and function code, (2) the forward question, and (3) the sanitized execution trace. The prompt instructs: "Narrate the execution trace step-by-step, explaining how the input is transformed into the output. Extract the \texttt{<Predicted Output>} you deduce from the trace."

Critically, the model is \textit{not generating an explanation from scratch}; it is \textit{translating} a factual trace into natural language. This ensures the rationale is verifiable: every claim can be cross-referenced against the trace.
...
\textbf{Why This Trace-Grounding Eliminates Hallucination.} Traditional self-instruct methods prompt an LLM to explain code without execution, leading to "plausible hallucinations" where the explanation sounds correct but contains subtle factual errors. By anchoring the rationale in a trace, we guarantee factual correctness: (1) variable values are factual (recorded at runtime), (2) control flow is factual (showing which branches executed), and (3) return values are factual (captured from actual execution).

\subsubsection{Step 6: Final Assembly into Conversational Format}
...
\subsubsection{Comparison to Alternative Verification Methods}
To contextualize the effectiveness of Dual Agreement, we compare it to alternative verification strategies on the same 5k validation set:

\textbf{vs. Single Execution (67\% accuracy).} A naive approach executes each generated solution against a single test case and accepts solutions that pass. This is highly brittle: a single flaky test (e.g., due to floating-point precision, randomness, or environmental dependencies) causes false rejections. Dual Agreement achieves 92\% accuracy, a 25-point improvement, by being robust to flaky tests—incorrect tests reduce $|T_p|$ for the correct cluster but do not eliminate it if other valid tests exist.

\textbf{vs. Formal Verification (impractical at scale).} Formal methods (e.g., symbolic execution, SMT solvers) can provide mathematical guarantees of correctness but are computationally expensive, often requiring minutes per program, and struggle with complex control flow or library calls. For large-scale synthesis (85,000 problems), formal verification is impractical. Dual Agreement achieves comparable empirical reliability (92\%) while scaling to large datasets with embarrassingly parallel execution.

\textbf{vs. LLM-as-Judge (non-deterministic).} An alternative is prompting a powerful LLM to judge solution correctness by analyzing code. However, this is (1) \textit{non-deterministic}: different sampling runs may yield different judgments, and (2) \textit{unsound}: the LLM may hallucinate correctness for plausible-looking but incorrect code. Dual Agreement is deterministic (execution results are reproducible) and grounded in operational semantics, not model intuition.

\textbf{vs. CodeT (outcome verification only).} Our work extends CodeT~\cite{chen2022codetcodegenerationgenerated} in a critical way. CodeT uses Dual Agreement to verify that generated \textit{code solutions} are correct (i.e., they pass tests). We apply the same principle but go further: after identifying verified code, we generate CoT rationales by translating execution traces, thereby verifying the \textit{reasoning process itself}, not just the final outcome. This distinction is essential for training models on trustworthy reasoning chains.

These results validate that Dual Agreement provides a practical, scalable, and empirically reliable verification method for large-scale synthetic data generation, forming a solid foundation for the training experiments that follow.
