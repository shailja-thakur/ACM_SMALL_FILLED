...
\end{lstlisting}
\vspace{-0.15cm}
\textbf{Extracted Metadata:}\\
\textbullet~Name: \texttt{solution}\\
\textbullet~Params: \texttt{freq\_list}\\
\textbullet~Input type: \texttt{list[tuple[str,int]]}\\
\textbullet~Output type: \texttt{dict[str,str]}
  &
  \begin{lstlisting}[style=pythoncode, basicstyle=\ttfamily\tiny]
Class: HuffmanTree;
__init__(self, freq_list:
...
\textbullet~Constructor: \texttt{\_\_init\_\_}\\
\textbullet~Methods: \texttt{build\_tree}, \texttt{get\_encoding}\\
\textbullet~Member params: \texttt{freq\_list}\\
\textbullet~Return types: \texttt{None}, \texttt{tuple}, \texttt{dict}
  \\
  \bottomrule
  \end{tabular}
  \vspace{-0.1cm}
  \caption{Signature format templates with extracted metadata. The LLM generates formal signatures specifying types and names. We extract and store this metadata (function/class names, parameters, input/output types) to validate generated code and tests for naming consistency and type correctness, preventing hallucinated function names or type mismatches.}
